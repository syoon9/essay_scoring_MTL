...
pos_tag_labels = ['Intro', 'R1', 'R2', 'CL', 'OT', 'SHELL', 'Q']
deprel_labels = ['X', 'COR', 'UNK']
score_labels = [0, 1, 2, 3, 4]
native_language_labels = [0, 1, 2, 3, 4]
...
class TSVProcessor(object):
    ...
    def get_train_examples(self, data_dir):
        ...
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "G20_combined_train_20000.tsv"), self.score_col_name), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "G20_combined_validate_20000.tsv"), self.score_col_name), "valid")

    def get_test_examples(self, data_dir):
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "G20_combined_test_rest.tsv"), self.score_col_name), "test")
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            if i == 0:
                ...
                continue
            id = line[columns['response_id']]
            tokens = line[columns['sentence']]
...
def convert_examples_to_features(examples, model, max_seq_length, special_tokens, logger,
    ...
    if not 'reg' in scoring_model_type :
        score_label_map = {label: i for i, label in enumerate(score_labels)}
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if 'reg' in scoring_model_type:
            ...
            score = score_label_map[str(example.score)]
...
def load_and_cache_examples(model, data_dir, max_seq_length, special_tokens, score_col_name, logger, vocab=None, tokenizer=None, evaluate=False, test=False, reload=False, scoring_model_type='reg'):
    ...
    if not reload and os.path.exists(cached_features_file):
        ...
        features = convert_examples_to_features(examples, model, max_seq_length, special_tokens, logger, vocab, tokenizer, scoring_model_type=scoring_model_type)
    ...
    if 'reg' in scoring_model_type:
         ...
         all_scores = torch.tensor([f.score for f in features], dtype=torch.long)
