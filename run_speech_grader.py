...
def get_auxiliary_objectives(args, vocab_size):
    if 'reg' in args.scoring_model_type:
        cls_num = 1
    else:
        cls_num = int(args.max_score)+1
    if 'tag' in args.scoring_model_type:
        score_alpha = args.score_alpha
    else: 
        score_alpha = 1.0
    training_objectives = {'score': (cls_num, score_alpha)}
    print('score objective', training_objectives['score'])
